            ...
            with open(feat_path, 'r', encoding='utf-8') as file:
                ...
                for line in file:
                    ...
                    if len(parts) >= 3:
                        ...
                        # Store only native tags with item name (GeTags only in session summary)
                        if native_tags:
                            token_text[item_id] = f"{item_name} ({' '.join(native_tags)})"
                        else:
                            token_text[item_id] = item_name

                        # Store preference tags separately for session-level aggregation
                        if preference_tags:
    ...
    def construct_prompt(self, dataset_name, user_his_text, candidate_text_order, session_pref_counts=None, candidate_prefs=None):
        ...
        if dataset_name in ['ml-1m', 'ml-1m-full']:
            ...
            if session_pref_counts and len(session_pref_counts) > 0:
                ...
                pref_section = f"基於您的購買歷史,這個會話顯示出以下顧客偏好模式:\n{pref_summary}\n\n"
                prompt = f"{pref_section}我過去按照順序購買了以下食品:\n{user_his_text}\n\n" \
                        f"現在有 {self.recall_budget} 個候選食品可以考慮接下來購買:\n{candidate_display}\n" \
                        f"請根據我的購買歷史,對這 {self.recall_budget} 個食品進行排序,衡量我最有可能接下來購買的食品。請逐步思考。\n" \
                        f"請使用順序編號顯示您的排序結果。用換行符分隔輸出。您必須對給定的候選食品進行排序。您不能生成不在給定候選列表中的食品。"
            else:
                # No preference summary, just show history
            ...
            if session_pref_counts and len(session_pref_counts) > 0:
                pref_summary = "\n".join([f"  - {pref}: {count} items" for pref, count in session_pref_counts.most_common()])
                pref_section = f"Based on your viewing history, this session shows the following viewer preference patterns:\n{pref_summary}\n\n"
                prompt = f"{pref_section}I've watched the following movies in the past in order:\n{user_his_text}\n\n" \
                        f"Now there are {self.recall_budget} candidate movies that I can watch next:\n{candidate_display}\n" \
                        f"Please rank these {self.recall_budget} movies by measuring the possibilities that I would like to watch next most, according to my watching history. Please think step by step.\n" \
                        f"Please show me your ranking results with order numbers. Split your output with line break. You MUST rank the given candidate movies. You can not generate movies that are not in the given candidate list."
            else:
            ...
            if session_pref_counts and len(session_pref_counts) > 0:
                pref_summary = "\n".join([f"  - {pref}: {count} items" for pref, count in session_pref_counts.most_common()])
                pref_section = f"Based on your purchase history, this session shows the following shopper preference patterns:\n{pref_summary}\n\n"
                prompt = f"{pref_section}I've purchased the following products in the past in order:\n{user_his_text}\n\n" \
                        f"Now there are {self.recall_budget} candidate products that I can consider to purchase next:\n{candidate_display}\n" \
            ...
            if session_pref_counts and len(session_pref_counts) > 0:
                pref_summary = "\n".join([f"  - {pref}: {count} items" for pref, count in session_pref_counts.most_common()])
                pref_section = f"Based on your visit history, this session shows the following visitor preference patterns:\n{pref_summary}\n\n"
                prompt = f"{pref_section}I've visited the following businesses in the past in order:\n{user_his_text}\n\n" \
                        f"Now there are {self.recall_budget} candidate businesses that I can consider to visit next:\n{candidate_display}\n" \
                        f"Please rank these {self.recall_budget} businesses by measuring the possibilities that I would like to visit next most, according to my visit history. Please think step by step.\n" \
