...
def fast_birank(
    ...
    # Adaptive tolerance and max_iters for large datasets
    # For very large datasets (>100k items), use more relaxed convergence
    if n_items > 100000:
        if tol == 1e-12:  # Only adjust if using default
            tol = 1e-6
            if verbose:
                print(f"Large dataset ({n_items:,} items) detected - using relaxed tolerance: {tol}")
        if max_iters == 1000:  # Only adjust if using default
            max_iters = 300
            if verbose:
                print(f"Large dataset - reducing max iterations to {max_iters}")
    ...
    if verbose:
        print(f"BiRank precomputation: matrix size {n_items}x{n_items}, tol={tol}, max_iters={max_iters}")
    ...
    with tqdm(
        ...
        for iter_num in pbar:
            ...
            pbar.set_description(
                f'BiRank iter {iter_num+1}/{max_iters}, step_size={step_size:.2e}, tol={tol:.2e}'
            )
            if step_size < tol:
                if verbose:
                    print(f"BiRank converged at iteration {iter_num+1}/{max_iters}")
                break
        else:
            if verbose:
                print(f"BiRank reached max iterations ({max_iters}) without convergence (final step_size={step_size:.2e})")
    del cur_transistion
...
class BiRankRecPredictor(BaseRecPredictor):
    ...
    def __init__(
        ...
        query_type: Union[Literal['union', 'intersection', 'sum', 'last'], Callable] = 'union',
...
class PageRankRecPredictor(BaseRecPredictor):
    ...
    def __init__(
        self,
        tag_corpous: Union[list[list[str]], sparse.spmatrix],
