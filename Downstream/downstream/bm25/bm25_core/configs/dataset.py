...
XY = Union[tuple[_X_T, _Y_T], tuple[_X_T], _Y_T, _PredictionDomain_T]
...
class UnseenLastItemsPredictionDatasetConfig(BaseDatasetConfig):
    ...
    def _init_dataset(self, global_config, verbose=False):
        ...
        for split, df in dfs.items():
            ...
            y = utils.seqs_to_onehot(y, len(all_pids))
            datasets[split] = (x, y)
        return datasets
...
class RetrievalTaskDatasetConfig(BaseDatasetConfig):
    ...
    max_n_relvants: Optional[int] = None
    ...
    def _init_dataset(self, global_config, verbose=False):
        ...
        for split, df in dfs.items():
            x, y = utils.preparation_for_session_based_retrieval_task(
                utils.pid_seqs_to_iid_seqs(
                    df['loaded_pids'].tolist(), all_pids=all_pids
                ),
...
class UserItemLastItemPredictionDatasetConfig(BaseDatasetConfig):
    ...
    max_in_seq_len: Optional[int] = None
    """minimum length of the input sequences (x). Sequences with x having length
    ...
    def _init_dataset(self, global_config, verbose=False):
        ...
        data_getter = _recursive_getattr(
            data_loaders, self.name[:-len('.last')], None
        )
        ...
        results = utils.preparation_for_ui_based_last_item_prediction(
            {
                split:
        ...
        for split, (x, y, y_pos) in results.items():
            ...
            if not self.repeatable:
                prediction_domain = ~utils.seqs_to_onehot(
                    y_pos, len(all_pids)
                )
    ...
    def check_repetable(self):
        # Allow repeatable=True for all datasets including movielens
        # The original constraint was too restrictive for session-based data
        return self
...
class UserItemLastItemRetrievalDatasetConfig(
    ...
    name: Literal['recipe.mini_leave_one_last_item.lastir',
                  ...
                  'yelp.leave_one_last_item.lastir']
    ...
    def _init_dataset(self, global_config, verbose=False):
        ...
        data_getter = _recursive_getattr(
            data_loaders, self.name[:-len('.lastir')], None
        )
...
DatasetConfigT = Annotated[
    Union[UnseenLastItemsPredictionDatasetConfig,
