...
_CACHE_DIR = 'data/preprocessed/yelp/bm25'

VariantT = Literal['leave_one_last_item']
...
def load_df(cache_dir: str = _CACHE_DIR, auto_split: bool = True, variant: VariantT = None):
    ...
    Like Amazon, we load full sequences (not pre-split).
...
def inters_df(
    ...
    *,
    ...
    # Perform leave-one-last-item split at load time (like Amazon)
    if split == 'train':
    ...
    # Convert to IIDs if requested (AFTER splitting, like Amazon)
    if use_iid:
...
def all_pids(cache_dir: str = _CACHE_DIR, *, variant: VariantT = None):
    """Returns all business IDs (in order of their integer index)"""
    assert variant in get_args(VariantT)
    # Map variant name to directory name
    if variant == 'leave_one_last_item':
        dir_name = '.'
    ...
    with open(
        ...
        # Return business IDs in index order for creating pid_to_iid mapping
        smap = json.load(fin)
        # smap is {"0": "business_id1", "1": "business_id2", ...}, return values in key order
...
def base_tags(
    ...
    *,
    ...
    if use_pid:
        # Convert to business_id-indexed dict
...
def check_integrity(cache_dir: str = _CACHE_DIR):
    ...
    for variant in ['leave_one_last_item']:
...
class _VariantHelper:
    ...
    def __getattr__(self, __name: str):
        ...
        assert fn is not None, f'data: {__name} for yelp does not exist'
