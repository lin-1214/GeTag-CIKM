...
class UniSRec(SASRec):
    def __init__(self, config, dataset):
        ...
        assert self.train_stage in [
            'pretrain', 'inductive_ft', 'transductive_ft', 'item'
        ...
        if self.train_stage in ['inductive_ft', 'transductive_ft', 'item']:
    ...
    def negative_sampling(self, pos_items, total_items, sample_num, distribution='uniform'):
        ...
        if distribution == 'uniform':
            ...
            for i in range(batch_size):
                print(i)
                # Uniformly sample `sample_num` negative items that are not in `pos_items`
                neg_sample = set()
                while len(neg_sample) < sample_num:
                    rand_idx = random.randint(0, total_items - 1)
                    if rand_idx not in pos_items:
                        neg_sample.add(rand_idx)
                neg_items.append(list(neg_sample))
            return torch.tensor(neg_items, device=pos_items.device)
        else:
    ...
    def full_sort_predict(self, interaction):
        """
        Generate scores for the given user sequence with negative sampling.
        If negative sampling is enabled, positive and sampled negative items are evaluated.
        """
        item_seq = interaction[self.ITEM_SEQ]
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        item_emb_list = self.moe_adaptor(self.plm_embedding(item_seq))
        ...
        # Retrieve item embeddings for the test phase
        ...
        # Extract positive item IDs for the batch
        pos_items = interaction[self.POS_ITEM_ID]  # Assuming positive item IDs are in this field

        # Use negative sampling for evaluation
        if True:
            # Number of negative samples to consider
            sample_num = 100
            total_items = test_items_emb.size(0)

            # Generate negative samples for each user in the batch
            batch_size = item_seq.size(0)
            neg_samples = []
            pos_items = pos_items.cpu().numpy()  # Convert to numpy for easier processing

            for i in range(batch_size):
                neg_sample = set()
                # Add the positive item to the sampled items
                neg_sample.add(pos_items[i])

                # Sample additional negative items that are not in the positive set
                while len(neg_sample) < sample_num:
                    rand_idx = random.randint(0, total_items - 1)
                    if rand_idx not in neg_sample:
                        neg_sample.add(rand_idx)

                neg_samples.append(list(neg_sample))

            # Convert to tensor and retrieve the corresponding embeddings
            neg_samples = torch.tensor(neg_samples, device=item_seq.device)
            sampled_test_items_emb = test_items_emb[neg_samples]  # Shape: [B, sample_num, D]

            # Compute scores only for the sampled items
            seq_output_expanded = seq_output.unsqueeze(1)  # Shape: [B, 1, D]
            scores = torch.bmm(seq_output_expanded, sampled_test_items_emb.transpose(1, 2)).squeeze(1)

            # Pad the scores tensor to match the full size [B, self.tot_item_num]
            padded_scores = torch.zeros((seq_output.size(0), self.tot_item_num), device=scores.device)
            # Assign the sampled scores to their respective positions in the padded_scores tensor
            for i in range(batch_size):
                padded_scores[i, neg_samples[i]] = scores[i]
            return padded_scores  # Ensure the shape matches [-1, self.tot_item_num]
        else:
            # Full sort prediction for all items
