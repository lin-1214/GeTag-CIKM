...
class RecommendationGPTTrainGeneratorBatch(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # If the user has no past interactions, we cannot sample.
        # Return a special value that the collate_fn can filter out.
    ...
    def collate_fn(self, batch):
        ...
        Custom collate function to encode and pad the batch of texts.
        """
        # Filter out the None values returned from __getitem__ for users with no interactions
        batch = [item for item in batch if item is not None]
        if not batch:
            # If the entire batch was empty, return empty tensors to avoid crashing the training loop
