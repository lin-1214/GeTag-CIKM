...
def _get_file(file_name: str, cache_dir):
    # Prefer an explicit directory via env; otherwise try common repo paths
    # Get the directory of this file
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Project root: src/tmp/final/data -> src/tmp/final -> src/tmp -> src -> PseudoUser
    project_root = os.path.normpath(os.path.join(current_dir, '..', '..', '..', '..'))
    # Allow override via environment variable
    env_dir = os.getenv('DATA_PKL_DIR', '').strip()
    candidate_dirs = []
    if env_dir:
        candidate_dirs.append(env_dir)
    # Common defaults (new first, then legacy)
    candidate_dirs.append(os.path.join(project_root, 'data', 'food', 'pkl'))
    candidate_dirs.append(os.path.join(project_root, 'data', 'pkl'))
    ...
    assert file_path is not None, (
        'Cannot find dataset file "{}". Tried: {}'.format(
            file_name, [os.path.join(d, file_name) for d in candidate_dirs]
        )
    )
