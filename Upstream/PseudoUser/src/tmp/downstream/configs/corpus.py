...
class ParsingConfig(BaseConfig):
    ...
    include_input_line: bool | None = None
    n_beams: int | None = None

    def init(
        self,
        raw_predicts: Mapping[int, str] | Mapping[int, list[str]],
        global_config,
        verbose=False,
    ):
...
class CorpusConfig(BaseConfig):
    ...
    postprocessing_config: PostProcessingConfig | None = Field(None)
    parsing_config: ParsingConfig | None = Field(None)

    add_n_pid_tags: int | None = Field(None)
