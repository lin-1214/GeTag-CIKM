...
def finetune_one_data(dataset, lambda_V, model_name, model_path, device, num_epochs=2, data_root=None,
                      early_stop=False, patience=5, min_delta=1e-3, monitor_metric="combined"):
    ...
    print(f"early_stop: {early_stop}, patience: {patience}, min_delta: {min_delta}, monitor: {monitor_metric}")
    ...
    best_sum = -float('inf')
    # Unified best score for the selected monitor metric (maximize)
    best_monitored_score = -float('inf')
    no_improve_epochs = 0
    ...
    for epoch in range(num_epochs):
        ...
        cur_sum = cur_recall_10 + cur_NDCG_10
    
        # Update trackers for reporting
        ...
        # Compute current monitored score (maximize)
        if monitor_metric == "val_loss":
            monitored_current = -val_rec_loss  # lower is better -> invert to maximize
        elif monitor_metric == "recall":
            monitored_current = cur_recall_10
        elif monitor_metric == "ndcg":
            monitored_current = cur_NDCG_10
        else:  # combined
            monitored_current = cur_sum

        # Save best checkpoint following monitor_metric
        if monitored_current > best_monitored_score + min_delta:
            best_monitored_score = monitored_current
            ...
            print(f"Saved best rec model (monitor={monitor_metric}) to {rec_model_dir}")
            no_improve_epochs = 0
        elif early_stop:
            no_improve_epochs += 1
            print(f"No improvement ({no_improve_epochs}/{patience}) on {monitor_metric}")
        ...
        print(f"Cur Recall@10: {cur_recall_10:.4f} / Best Recall@10: {best_recall_10:.4f}")
        print(f"Cur NDCG@10: {cur_NDCG_10:.4f} / Best NDCG@10: {best_NDCG_10:.4f}")    
...
def finetune(num_dataset, lambda_V, device, model_name="Qwen/Qwen3-0.6B", model_path="models", num_epochs=2, data_root=None,
             early_stop=False, patience=5, min_delta=1e-3, monitor_metric="combined"):
    for i in range(num_dataset):
        print(f"-----Finetuning dataset {i + 1}/{num_dataset}-----")
        try :
            finetune_one_data(
                f"user_session_data_{i}", lambda_V, model_name, model_path, device, num_epochs, data_root,
                early_stop=early_stop, patience=patience, min_delta=min_delta, monitor_metric=monitor_metric
            )
        except Exception as e:
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="../cllm_data")
    parser.add_argument("--num_epochs", type=int, default=2)
    parser.add_argument("--early_stop", action="store_true")
    parser.add_argument("--patience", type=int, default=5)
    parser.add_argument("--min_delta", type=float, default=1e-3)
    parser.add_argument("--monitor_metric", type=str, default="combined", choices=["combined","val_loss","recall","ndcg"])
