"""
Fine-tune UniSRec on i3fresh dataset

Usage:
    python src/finetune_i3fresh.py --dataset i3fresh --checkpoint data/checkpoints/UniSRec-FHCKM-300.pth
"""
...
# Add models/unisrec to path so checkpoint can load 'unisrec' module
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src', 'models', 'unisrec'))

from recbole.config import Config
from recbole.data import data_preparation
from recbole.utils import init_seed, init_logger, get_trainer, set_color

# Import from the new location but checkpoint will use old path
...
def finetune(dataset, pretrained_file, fix_enc=True, **kwargs):
    ...
    Args:
        dataset: dataset name (should match directory in data/unisrec/)
    ...
    # Configuration files
    props = [
        'src/models/unisrec/props/UniSRec.yaml',
        'src/models/unisrec/props/finetune.yaml'
    ]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Fine-tune UniSRec on i3fresh dataset')
    parser.add_argument(
        '--dataset', '-d', type=str, default='i3fresh',
        help='Dataset name (directory name in data/unisrec/)'
    )
    parser.add_argument(
        '--checkpoint', '-p', type=str,
        default='data/checkpoints/UniSRec-FHCKM-300.pth',
    ...
    print("="*80)
    print("FINE-TUNING UNISREC ON I3FRESH DATASET")
    print("="*80)
