...
def convert_data_for_cllm(input_csv, output_dir, dataset_name="user_session_data", class_column="Class", validation_ratio=0.1, test_ratio=0.1, min_user_interactions=2):
    ...
    # Helper to parse class labels (supports multi-label when enabled)
    def parse_labels(value):
        if not config.MULTI_LABEL:
            # Single-label path
            if isinstance(value, str):
                return [value]
            return [str(value)] if value is not None and not (isinstance(value, float) and np.isnan(value)) else ["unknown"]
        # Multi-label path
        ...
        if isinstance(value, str):
            ...
            return [p.strip() for p in s.split(config.LABEL_SEPARATOR) if p.strip()]
    ...
    for _, row in df.iterrows():
        ...
        class_value = row.get(class_column, "unknown")
        classes = parse_labels(class_value)
        session_id = None
        ...
        for i in range(120):  # Assuming columns 0-119 might contain session data
            ...
            # Store the LLM-generated user class(es)
            if classes and session_id:
                user_class[session_id] = classes if config.MULTI_LABEL else classes[0]
    ...
    for user_id, label_value in user_class.items():
        if user_id not in user_sessions:
            continue
        if config.MULTI_LABEL:
            for cls in label_value:
                class_to_users[cls].append(user_id)
        else:
            class_to_users[label_value].append(user_id)
    
    # Create user ID based solely on class number (each class gets ONE numerical ID)
    # Map original user IDs to their class-based numerical ID(s)
    class_id_map = {}
    class_based_user_map = defaultdict(list)
    ...
    for class_idx, class_name in enumerate(sorted(class_to_users.keys())):
        ...
        # Map all users of this class to this numerical ID (append for multi-label)
        for user_id in class_to_users[class_name]:
            class_based_user_map[user_id].append(class_idx)
    ...
    for orig_user_id, sessions in user_sessions.items():
        ...
        target_class_ids = class_based_user_map[orig_user_id]
        
        # Separate purchases and loads for each assigned class
    ...
    mappings = {
        ...
        "original_to_class_id": dict(class_based_user_map),  # Original user ID to list of numerical class IDs
    ...
    for i, (prompt_text, content_text) in enumerate(reviews):
        ...
        if "reviewed" in prompt_text:
            ...
            if len(parts) >= 3:
                ...
                try:
                    class_id = int(user_part.strip('<>').split('_')[1])
                    item_idx = int(item_part.strip('<>').split('_')[1])
                    class_name = class_id_to_name.get(class_id, "unknown")
...
def generate_prompt_files(file_path, cllm_data_path, cllm_processed_data_path):
    for i in range(config.K_AUG):
        classified_data_path=f"{file_path}_{i}.csv"

        if not os.path.exists(classified_data_path):
            print(f"File {classified_data_path} does not exist.")
            continue

        dataset_dir = convert_data_for_cllm(classified_data_path, cllm_data_path, f"{cllm_processed_data_path}_{i}", "Class")
        generate_cllm_prompt_files(dataset_dir) 

        inspect_dataset(dataset_dir)    # Debug use
